...
public class CPHInline
{
    ...
    public string createTitle(string name, string widgetScene){
        string titleId = JToken.Parse(CPH.ObsSendRaw("CreateInput", "{\"sceneName\":\""+widgetScene+"\",\"inputName\":\"pollTitle\",\"inputKind\":\"text_gdiplus_v2\",\"inputSettings\":{\"text\":\""+name.ToUpper()+"\",\"color\":"+textColor+", \"bk_opacity\": 60, \"font\":{\"size\":64}},\"sceneItemEnabled\":true}", 0)).Value<JToken>("sceneItemId").ToString();
        CPH.LogInfo($"created title {name}");
        return titleId;
    }
    public string createLabel(string name, string widgetScene, int index){
        string labelId = JToken.Parse(CPH.ObsSendRaw("CreateInput", "{\"sceneName\":\""+widgetScene+"\",\"inputName\":\""+ name +"_label\",\"inputKind\":\"text_gdiplus_v2\",\"inputSettings\":{\"text\":\""+index+": "+name.ToUpper()+"\",\"color\":"+textColor+", \"bk_opacity\": 60, \"font\":{\"size\":60}},\"sceneItemEnabled\":false}", 0)).Value<JToken>("sceneItemId").ToString();
    ...
    public void createBars(Dictionary<string, int> pollBarsDict, string widgetScene){
        ...
        foreach (var item in pollBarsDict)
        {
            ...
            CPH.ObsSendRaw("SetSceneItemTransform", "{\"sceneName\":\""+ widgetScene +"\",\"sceneItemId\":"+ labelId +",\"sceneItemTransform\":{\"positionX\":40,\"positionY\":"+ (posY+10) +"}}", 0);
            CPH.ObsSendRaw("SetSceneItemTransform", "{\"sceneName\":\""+ widgetScene +"\",\"sceneItemId\":"+ barId +",\"sceneItemTransform\":{\"positionX\":"+posX+",\"positionY\":"+posY+"}}", 0);
            CPH.ObsSendRaw("CreateSourceFilter", "{\"sourceName\":\""+item.Key+"\",\"filterName\":\"move\",\"filterKind\":\"move_value_filter\",\"filterSettings\":{\"setting_name\":\"width\"}}", 0);
            CPH.Wait(300);
    ...
    public void updateBars(){
        ...
        foreach (var item in pollBarsDict)
        {
            ...
            CPH.ObsSendRaw("SetSourceFilterSettings", "{\"sourceName\":\""+item.Key+"\",\"filterName\":\"move\",\"filterSettings\":{\"setting_int\":"+setWidth+",\"setting_name\":\"width\"},\"overlay\":true}", 0);
            CPH.Wait(100);
            CPH.ObsSendRaw("SetSourceFilterEnabled", "{\"sourceName\":\""+item.Key+"\",\"filterName\":\"move\",\"filterEnabled\":true}", 0);
        }
    }
